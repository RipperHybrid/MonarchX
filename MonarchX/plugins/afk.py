...
from MonarchX.helpers.help_func import get_arg
import MonarchX.MonarchX_db.afk_db as Zect
from MonarchX.helpers.help_func import user_afk
from MonarchX import get_readable_time
from MonarchX.helpers.utils import get_message_type, Types
...
@MonarchX.on_message(filters.command("afk", HANDLER) & filters.me)
async def afk(MonarchX, message):
...
@MonarchX.on_message(filters.mentioned & ~filters.bot & filters.create(user_afk), group=11)
...
@MonarchX.on_message(filters.create(user_afk) & filters.outgoing)
async def auto_unafk(_, message):
    await Zect.set_unafk()
    unafk_message = await MonarchX.send_message(message.chat.id, "**I'm no longer AFK**")
    global MENTIONED
    ...
    for x in MENTIONED:
        ...
        await MonarchX.send_message(GROUP_ID, text)
        MENTIONED = []
